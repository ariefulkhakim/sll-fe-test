...
interface ModalProps {
  ...
  id?: string;
  onClose: () => void;
  dataSpecies: ResponseSpecies | undefined;
}
const ModalUpdate = ({ open, onClose, dataSpecies }: ModalProps) => {
  const { logout } = useAuth();
  const [form] = Form.useForm();
  const queryClient = useQueryClient();

  const { mutateAsync: updateSpecies, isLoading } = useMutation({
    mutationFn: SpeciesService.updateSpecies,
    onSuccess: (res) => {
      console.log(res);
  ...
  useEffect(() => {
    form.setFieldsValue(dataSpecies);
  }, [form, dataSpecies]);
  ...
  return (
    <>
      <Modal
        open={open}
        onCancel={onClose}
        title={`Update Species ${dataSpecies?.scientificName}`}
        footer={null}
        width={850}
        style={{ top: 20 }}
      >
